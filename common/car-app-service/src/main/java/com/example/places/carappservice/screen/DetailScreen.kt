...
class DetailScreen(carContext: CarContext, private val placeId: Int) : Screen(carContext) {
    ...
    override fun onGetTemplate(): Template {
        ...
        val intent = Intent(CarContext.ACTION_NAVIGATE, Uri.parse("geo:0,0?q=$place"))
        val resolveInfo = carContext.packageManager.resolveActivity(intent, 0)
        ...
        val navigateAction = Action.Builder()
            ...
            .setOnClickListener { if (resolveInfo != null) {
                carContext.startCarApp(intent)
            } else {
                Log.w("CarApp", "No navigation app installed to handle intent")
                // You can show a message or prompt user to install one
            }}
            .build()
